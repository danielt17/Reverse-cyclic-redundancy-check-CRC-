...
def print_crc_parameters(crc_algorithm_name: str) -> int:
    '''
    Description:
    ...
    Outputs:
        None. Prints text.
    '''
    params: dict[str, Any] = get_algorithm_params(crc_algorithm_name)
    print('\nCyclic redundancy check parameters: \n')
    for _, param in enumerate(params.items()):
        if param[0] == 'poly':
            print('poly: ' + str(hex(int(param[1]))))
        else:
            print(str(param[0]) + ': ' + str(param[1]) + '.')
    print('\n')
    return int(params['width'])

def get_crc(data: bytes | bytearray, crc_algorithm: Callable[[bytes | bytearray], int]) -> bytes:
    '''
    Description:
