# -*- coding: utf-8 -*-
"""
Created on Fri Feb 18 15:47:46 2022
...
class PrimeField(Field):
    ...
    def __init__(self, mod):
        ...
        if mod < 2:
            ValueError("Modulus must be prime")
...
class Matrix:
    ...
    def reduced_row_echelon_form(self):
        ...
        for j in range(cols):  # For each column
            ...
            REF_recipe_ls.append([j,numpivots,pivotrow])
            self.swap_rows(numpivots, pivotrow)
            pivotrow = numpivots
            numpivots += 1
			
			# Simplify the pivot row
            val_pivot_row = self.f.reciprocal(self.get(pivotrow, j))
            REF_recipe_ls[-1].append(val_pivot_row)
            self.multiply_row(pivotrow, val_pivot_row)
			
			# Eliminate rows below
            multiple_values = []
            for i in range(pivotrow + 1, rows):
                multiple_value = self.f.negate(self.get(i, j))
                self.add_rows(pivotrow, i, multiple_value)
                multiple_values.append(multiple_value)
            REF_recipe_ls[-1].append(multiple_values)
        ...
        for i in reversed(range(numpivots)):
            ...
            RREF_recipe_ls.append([i,pivotcol])
			
			# Eliminate rows above
            neg_values = []
            for j in range(i):
                neg_value = self.f.negate(self.get(j, pivotcol))
                self.add_rows(i, j, neg_value)
                neg_values.append(neg_value)
            RREF_recipe_ls[-1].append(neg_values)
        return REF_recipe_ls,RREF_recipe_ls
